...
function buildGeneratePrompt({
  ...
  return [
    ...
    'External platform facts: write a concise Brandible sentence the approved evidence actually supports, then insert the matching {{AC#}} token immediately after it. Do not paste the evidence quote. Do not restate that sourced sentence in the next sentence. Surrounding commentary may be Brandible voice.',
    ...
    'When an allowed claim has citation required, insert its {{AC#}} token after the Brandible sentence it supports. Code replaces the token with [Source](url). Use at most one token per paragraph unless two distinct approved facts are both required.',
...
function buildRevisionPrompt(article, problems, { facts, catalog, sourcePack, topic, allowlist, allowedClaims }) {
  ...
  return [
    ...
    'For sourced-fact failures, write concise Brandible wording entailed by an approved claim and place the matching {{AC#}} token after it, delete the sentence, or remove an unused token. Do not invent a new sourced sentence. Do not dump the evidence quote as a giant Markdown link.',
